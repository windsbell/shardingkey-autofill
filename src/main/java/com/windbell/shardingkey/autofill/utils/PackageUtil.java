package com.windbell.shardingkey.autofill.utils;
...
public class PackageUtil {
     ...
     * 获取包下class集合
     * packageName:包名
     * isRecursive：是否递归，检索子包所有class
     * filter: 满足条件的过滤器
     */
    public static List<Class<?>> getClassList(String packageName, boolean isRecursive, Predicate<Class<?>> filter) {
        List<Class<?>> classList = getClassList(packageName, isRecursive);
        return classList.stream().filter(filter).collect(Collectors.toList());
    }
